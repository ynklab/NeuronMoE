...
class TorchModel:
    ...
    def _set_units_hook_wrapper(
        ...
        """
        特定のレイヤーの出力を監視/操作するためのhook
        forward_hook: forward pass実行時に呼ばれる
        backward_hook: backward pass実行時に呼ばれる
        register_hook: tensor gradientの計算時に呼ばれる
        """
    ...
    def set_units_in_layer(
        ...
        特定のレイヤーの指定されたユニットの値を設定
    
        Args:
            layer_name: 対象レイヤーの名前
            unit_ids: 制御したいユニットのインデックス
            values: 設定したい値
        """
        # レイヤー名から":0"を削除
            ...
            if iterated_module_name == layer_name:
                ...
                handle = layer.register_forward_hook(
                    self._set_units_hook_wrapper(
                        units=units,      # 制御したいユニットのインデックス
                        values=values,    # 設定したい値
                        only_last_token=only_last_token  # 最後のトークンのみ変更するかどうか
                    )
                )
                # フックのハンドルを保存（後で解除できるように）
    ...
    try:
        ...
        if rand_weights:
            ...
            if 'Llama-2' in model_name:
                ...
                m = AutoModelForCausalLM.from_pretrained(model_name, cache_dir=cache_dir, device_map="auto", torch_dtype="auto")
...
def get_layer_regex(model_name: str) -> t.Optional[t.List[str]]:
    ...
    if family == "gpt2":
        ...
        layer_types = [
            ...
            "model.layers.([0-9]|[0-9][0-9]).self_attn.o_proj",
...
def _collect_responses_info_for_model(model: TorchModel, model_family: str) -> t.List[ResponseInfo]:
    """
    Filters model response information objects based on specific layer criteria.
    
    Args:
        model: Neural network model instance with get_response_infos() method
        
    Returns:
        List[ResponseInfo]: Filtered response information objects meeting criteria:
            - Layer type is Conv1D, BertLayerNorm, or Linear
            - Response shape has 2 or 3 dimensions
            - Layer name does not contain "lm_head"
    """
    mapping = {
        "gpt2": [
            ri
