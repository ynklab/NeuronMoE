...
def argument_parser(prev_args: t.Optional[str] = None):
    ...
    parser.add_argument(
        "--japanese_control", type=str, default="", help="control Japanese neurons"
    )
    parser.add_argument(
        "--top-k",
        type=int,
    ...
    parser.add_argument("--expertise2", type=pathlib.Path, help="Expertise results as CSV file.")
    parser.add_argument(
        "--metric",
        type=str,
        default="ap",
            ...
            for force_layer in layer_names:
                for num_units in args.num_units:
                    ...
                    if num_units > 0:
                        ...
                        if args.japanese_control == "on":
                            expertise2 = pd.read_csv(args.expertise2)
                            model, df_force = force_units_hooks(
                                model=model,
                    ...
                    for seed in sweep_seed:
                        ...
                        if "translation_text_" in args.prompt:
                            ...
                            with open(args.prompt, 'rb') as f:
                                source_text = pickle.load(f)[seed-1]['ja']

                            if args.prompt_format_id_for_translation == 0:
                                prompt = f'以下の日本語の文章をターゲット言語に翻訳しなさい．\n日本語: {source_text}\nターゲット言語:'
                            elif args.prompt_format_id_for_translation == 1:
                                prompt = f'以下の日本語の文章を関西弁に翻訳しなさい．\n日本語: {source_text}\n関西弁:'
                            ...
                            elif args.prompt_format_id_for_translation == 8:
                                prompt = f'Translate a Japanese sentence into a target language.\nJapanese: {source_text}\nTarget language:'
                            else:
